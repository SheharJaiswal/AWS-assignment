...
{
    [HttpGet]
    public IActionResult Index() => View(storage.GetAll());
    [HttpGet]
    ...
    public async Task<IActionResult> Upload(DocumentUploadViewModel model, CancellationToken ct)
    {
        if (model.File is null)
            ModelState.AddModelError(nameof(model.File), "Please select a document.");
        ...
        try
        {
            await storage.SaveAsync(model.File!, model.DocumentType, ct);
