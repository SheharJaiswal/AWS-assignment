    ...
    IConfiguration config) : ILocalDocumentStorage
{
    ...
    private readonly long _maxFileSize =
        (config.GetValue<long?>("DocumentStorage:MaxFileSizeMb") ?? 10) * 1024L * 1024L;

    private readonly HashSet<string> _allowedExtensions =
        (config.GetSection("DocumentStorage:AllowedExtensions").Get<string[]>()
            ?? [".pdf", ".png", ".jpg", ".jpeg"])
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var stream = File.Create(Path.Combine(_root, storageKey));
