...
namespace AwsDocumentPortal.Services
{
    ...
    {
        readonly ConcurrentDictionary<Guid, DocumentItem> docs = new();
        readonly string root = Path.Combine(env.ContentRootPath, "App_Data", "uploads");
        ...
        public async Task<DocumentItem> SaveAsync(IFormFile file, string type, CancellationToken ct)
        {
            if (file.Length <= 0)
            ...
            Directory.CreateDirectory(root); var id = Guid.NewGuid();
            var key = $"{id:N}{ext.ToLowerInvariant()}";
            await using var s = File.Create(Path.Combine(root, key));
